...
.header 
  ...
  #listBtn(@click="listBtn()" )
    Icon(type="ios-arrow-down" size="25" color="grey")
  Uploadimgs(ref="zanImg" v-if="showUploadimgs == true")
    Menu#HidAndShow(ref="menu" :mode="mode" theme="dark" :collapse="collapse" 
      width="180px" :style="{ border: 'none' }"
    )
      RouterLink(to="/PageBuy")
        MenuItem.menu-item(name="1") 購買頁
      RouterLink(to="/PageOrder")
        MenuItem.menu-item(name="2") 我的訂單
      Submenu.menu-item(name="3")
        template(#title='')
          Icon(type="md-person") 
          Hi ruby
        MenuItem(name="3-1") 帳戶資料
        RouterLink(to="/PageOrder")
          MenuItem(name="3-2") 我的訂單紀錄
        MenuItem(name="3-3") 登出
</template>
